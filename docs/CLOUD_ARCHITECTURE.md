...
> **部署模式：** 本文档描述默认的 `agentcore` 模式。系统同时支持 `ecs` 模式，用于 AWS 中国区域（cn-north-1 / cn-northwest-1）部署。ECS 模式使用 ECS Fargate 代替 AgentCore microVM，自建 OIDC 认证服务代替 Cognito，Anthropic API 代替 Bedrock。通过 CDK context 变量 `mode=ecs` 切换。
...
| Agent 运行时 | AgentCore (global) / ECS Fargate (中国区) | 全球使用 microVM 隔离，中国区使用 ECS 共享服务 + ABAC 隔离 |
...
└──────┬───────┘                        └────────┬────────────┘
       ...
       ▼                                         │
┌──────────────────────────────────────────────────────────────┐
│     ECS Fargate (Control Plane) — 同 agentcore 模式           │
│     JWT 验证改用 JWKS（指向 Auth Service）                      │
│     Agent 调用改用 HTTP POST（指向 Agent Service ALB）          │
└──────────────────────┬──────────────────┬────────────────────┘
                       │                  │
          HTTP POST /invocations   HTTP /auth/*
                       ▼                  ▼
┌──────────────────────────┐  ┌────────────────────────────────┐
│  ECS Fargate             │  │  ECS Fargate                   │
│  Agent Service (内网ALB)  │  │  Auth Service (内网ALB)         │
│  ├── Claude Agent SDK    │  │  ├── RS256 JWT 签发/验证         │
│  │   └── Anthropic API   │  │  ├── bcrypt 密码哈希             │
│  ├── MCP 工具             │  │  ├── JWKS Endpoint             │
│  ├── S3 Session 同步      │  │  └── 管理员 API (创建/禁用用户)  │
│  └── STS ABAC 隔离        │  └────────────────────────────────┘
└──────────────────────────┘
```

> **ECS 模式与 agentcore 模式的关键区别：**
> - Agent 运行时：microVM per-session → ECS 共享服务（单并发 per task + ABAC 数据隔离）
> - 认证：Cognito User Pool → 自建 OIDC 服务（RS256 JWT, DynamoDB 用户存储）
> - LLM：Bedrock → Anthropic API（通过 Credential Proxy 或直接 API Key）
> - 会话持久化：AgentCore session → 纯 S3 session sync（无状态 per request）
...
| - | ECS 模式 (新增) | AgentCore/Cognito/Bedrock → ECS/自建Auth/Anthropic API，支持中国区部署 |
